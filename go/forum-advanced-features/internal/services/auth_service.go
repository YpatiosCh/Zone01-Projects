...
func NewAuthService(repo *repository.Manager) AuthInterface {
	return &AuthService{
		repo: repo,
	}
}
...
func (s *AuthService) RegisterUser(username, email, password string) (string, Error) {
	...
	if existingUsers != nil && len(existingUsers) > 0 {
		...
		provider := user["oauth_provider"].(string)
		if provider != "" {
			Err = Error{Msg: "Email registered with " + provider, Code: 400}
		} else {
			Err = ErrEmailExists
		}
		return "", Err
...
func (s *AuthService) Login(email, password string) (string, Error) {
	...
	if !ok {
		oauth, ok := user["oauth_provider"].(string)
		if ok && oauth != "" {
			Error := Error{Msg: "User registered with " + oauth, Code: 400}
			return "", Error
		}
		Error := Error{Msg: "No password", Code: 400}
		return "", Error
	}
