...
// DeleteJunctionByColumn deletes records from a junction table where a specific column matches a value
func (m *Manager) DeleteJunctionByColumn(table, column string, value interface{}) error {
	// Validate input
	if table == "" {
		return fmt.Errorf("table name cannot be empty")
	}
...
// GetByTwoColumns retrieves rows matching two column conditions
func (m *Manager) GetByTwoColumns(table, col1 string, val1 interface{}, col2 string, val2 interface{}) ([]map[string]interface{}, error) {
	query := fmt.Sprintf("SELECT * FROM %s WHERE %s = ? AND %s = ?",
		strings.ToLower(table),
		strings.ToLower(col1),
		strings.ToLower(col2))

	rows, err := m.Db.Query(query, val1, val2)
	if err != nil {
		return nil, fmt.Errorf("failed to query table %s: %w", table, err)
	}
	defer rows.Close()
...
func (m *Manager) GetUserIDbyUsername(username string) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("failed to query user ID by username: %w", err)
	}
