CREATE TABLE IF NOT EXISTS user (
    ...
    password TEXT,
    oauth_provider TEXT,
    --'google', 'github', or NULL for regular users
...
CREATE TABLE IF NOT EXISTS notification (
    ...
    comment_id TEXT,
    -- Related comment (nullable)
    message TEXT NOT NULL,
    -- Notification message
    read BOOLEAN DEFAULT 0,
    -- Read status (using 0/1 for SQLite)
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES user(id),
...
CREATE INDEX IF NOT EXISTS idx_user_oauth ON user(oauth_provider, oauth_provider_id);
