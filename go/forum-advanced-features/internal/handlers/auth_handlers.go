...
func RegisterUser(service services.Services, tmpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPost {
			...
			userID, err := service.Auth().RegisterUser(username, email, password)
			if err.Code != 0 {
...
func LoginUser(service services.Services, tmpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPost {
			...
			sessionID, err := service.Auth().Login(email, password)
			if err.Code != 0 {
...
func LogoutUser(service services.Services, tmpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		Error := service.Auth().Logout(sessionID)
		if Error.Code != 0 {
