package piscine

// BTreeApplyPreorder applies a given function to each node in the binary tree in a pre-order traversal.
// Pre-order traversal visits the root node first, then the left subtree, and finally the right subtree.
// This function is useful for performing operations on each node of the tree in a specific order,
// often used for tasks that require processing the tree's nodes in a top-down manner.
func BTreeApplyPreorder(root *TreeNode, f func(...interface{}) (int, error)) {
	// Check if the current node is empty or nil.
	// If it is, return immediately as there's nothing to do.
	if root == nil {
		return
	}
	// Apply the given function to the data part of the root (or current node).
	// This is where the actual operation on the node's data is performed.
	f(root.Data)
	// Traverse the left subtree by recursively calling the pre-order function.
	// This ensures that all nodes in the left subtree are visited after the root.
	BTreeApplyPreorder(root.Left, f)
	// Traverse the right subtree by recursively calling the pre-order function.
	// This ensures that all nodes in the right subtree are visited after the root.
	BTreeApplyPreorder(root.Right, f)
}
