...
export function mockServerPlugin() {
  return function plugin({ store, router, events }) {
    ...
    const unsub = events.on("wss:send", (msg) => {
...
1. Views emit `events.emit("wss:send", { type: "MOVE_PLAYER_1" })` — this represents a client sending a message
2. The plugin receives the message via `events.on("wss:send", ...)` — it acts as the "server"
